...
-- Descripción: Verificación y creación de tablas para garantizar consistencia
-- Autor: Sistema
-- Fecha: 2025-10-06
-- =============================================
...
-- Tabla: perfil_opcionmenu
CREATE TABLE IF NOT EXISTS `perfil_opcionmenu` (
    `Id_RolOpcionMenu` INT NOT NULL AUTO_INCREMENT,
    `Id_Perfil` INT NOT NULL,
    `Id_OpcionMenu` INT NOT NULL,
    PRIMARY KEY (`Id_RolOpcionMenu`),
    KEY `Id_Perfil` (`Id_Perfil`),
    KEY `Id_OpcionMenu` (`Id_OpcionMenu`),
    CONSTRAINT `perfil_opcionmenu_ibfk_1` FOREIGN KEY (`Id_Perfil`) REFERENCES `roles_perfil` (`Id_Perfil`),
    CONSTRAINT `perfil_opcionmenu_ibfk_2` FOREIGN KEY (`Id_OpcionMenu`) REFERENCES `opcionmenu` (`Id_OpcionMenu`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci;
